...
def letter_distribution(words):
    """
    Returns a pd.Series keyed by letters, giving the fraction of elements in
    'words' containing each letter.
    """
    frequency_table = Counter()
    for word in words:
        frequency_table.update(set(list(word)))

    # relative_frequency is the percentage of words containing each letter.
    relative_frequency = pd.Series(frequency_table) / len(words)
...
class WordleFilter:
    ...
    def excludes(self, letters):
        keep = [x for x in self.words if not set(x).intersection(set(letters))]
        return WordleFilter(keep)

    def letter_not_in(self, letter, position):
        if (isinstance(position, int)):
            position = (position, )
        keep = self.words
        for pos in position:
            keep = [x for x in keep if letter in x and x[pos] != letter]
        # keep = [x for x in self.words if letter in x
        #         and x[position] != letter]
        return WordleFilter(keep)

    def letter_in(self, letter, position):
        keep = [x for x in self.words if x[position] == letter]
        return WordleFilter(keep)

    def __repr__(self):
        return f"{len(self.words)} words"
